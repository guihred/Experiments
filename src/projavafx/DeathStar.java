...
public class DeathStar extends Application {
	...
	private static final int DIVISION = 100;// the bigger the higher resolution
...
//		uncomment if you want to move the other sphere
	...
	static TriangleMesh createMesh(final int division, final float radius, final Point3D centerOtherSphere) {
		...
		for (int y = 0; y < div2 - 1; ++y) {
			...
			for (int i = 0; i < division; ++i) {
				...
				checkOtherSphere(radius, centerOtherSphere, points, pPos);
		...
		points[pPos + 5] = 0;
		checkOtherSphere(radius, centerOtherSphere, points, pPos + 3);
		pPos += 6;
	...
	private static void checkOtherSphere(final float radius, final Point3D centerOtherSphere, float[] points, int pPos) {
	...
	public static void main(String[] args) {
		int pontas = 5;
		int radius = 70;
		double[] points = DoubleStream
				.iterate(0, i -> i + 1)
				.limit(pontas * 2)
				.flatMap(
						i -> DoubleStream.of(2 * radius + radius * (i % 2 + 1) * Math.cos(Math.toRadians(i * 360 / pontas / 2)),
								radius * (i % 2 + 1) * Math.sin(Math.toRadians(i * 360 / pontas / 2)))).toArray();
		String collect = DoubleStream
				.iterate(0, i -> i + 1)
				.limit(pontas * 2)
				.mapToObj(
						i -> "L" + radius * (i % 2 + 1) * Math.cos(Math.toRadians(i * 360 / pontas / 2)) + "," + radius * (i % 2 + 1)
								* Math.sin(Math.toRadians(i * 360 / pontas / 2))).collect(Collectors.joining(" "));
