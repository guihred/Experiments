...
public class DotsModel {
			...
			if (selected != null && over != null && selected != over
				...
				if (!collect1.isEmpty()) {
					...
					for (Set<DotsSquare> collect2 : collect1) {
						List<Double> map = map(flatMap(collect2, a -> Arrays.asList(a.getCenter())),
					...
					while (currentPlayer == 0) {
						...
						if (!collect3.isEmpty()) {
							...
							for (Set<DotsSquare> q : collect3) {
								...
								List<Double> map = map(flatMap(q, a -> Arrays.asList(a.getCenter())),
										(Double d) -> Double.valueOf(d));
								final Double[] toArray = map.toArray(new Double[map.size()]);
								final Polygon polygon = new Polygon(doubleArray(toArray));
								polygon.setFill(colors[currentPlayer]);
	...
	private List<Map.Entry<DotsSquare, DotsSquare>> getMelhorPossibilidades() {
		...
		for (int i = 0; i < MAZE_SIZE; i++) {
			for (int j = 0; j < MAZE_SIZE; j++) {
				...
				final List<Map.Entry<DotsSquare, DotsSquare>> collect = map(checkMelhor,
						e -> new AbstractMap.SimpleEntry<>(maze1, e));
	...
	private List<Map.Entry<DotsSquare, DotsSquare>> getMelhorPossibilidades2() {
		...
		final List<Map.Entry<DotsSquare, DotsSquare>> collect = filter(possibilidades,
				(Map.Entry<DotsSquare, DotsSquare> entry) -> {
	...
	private List<Map.Entry<DotsSquare, DotsSquare>> getMelhorPossibilidades3() {
		...
		final Map<Integer, List<Map.Entry<DotsSquare, DotsSquare>>> collect = groupBy(possibilidades,
				e -> getCountMap(e.getKey(), e.getValue()));
