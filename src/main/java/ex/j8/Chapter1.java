...
public final class Chapter1 {
    ...
    public static void ex2(File directory) {
        ...
        Arrays.asList(directory.listFiles(File::isDirectory)).forEach(s -> LOGGER.trace("{}", s));
        Arrays.asList(directory.listFiles(File::isDirectory)).forEach(s -> LOGGER.trace("{}", s));
     ...
     * turns them into unchecked exceptions. For example, new Thread(uncheck( () ->
     * { LOGGER.trace("Zzz"); Thread.sleep(1000); })).start(); // Look, no catch
     * (InterruptedException)! Hint: Define an interface RunnableEx whose run method
     * may throw any exceptions. Then implement public static Runnable
     * uncheck(RunnableEx runner). Use a lambda expression inside the uncheck
     * function. Why can't you just use Callable<Void> instead of RunnableEx?
     * 
     ...
     * such as this one? String[] names = { "Peter", "Paul", "Mary" };
