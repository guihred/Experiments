...
// TODO
        ...
        return new VBox(new SplitPane(build, tableView));
    }
    ...
    private void getNewLinks(TreeItem<Map.Entry<String, String>> newValue, Set<String> links,
        ...
        CompletableFuture.supplyAsync(SupplierEx.makeSupplier(() -> {
            ...
            return getLinks(doc, entry, domain, links, level);
        }).thenAccept(l -> {
            LOG.info("Links {}", l);
            links.addAll(l.stream().map(e -> e.getValue()).collect(Collectors.toList()));
