...
public final class ImprovedNoise {
    ...
    static {
        for (int i = 0; i < permutation.length; i++) {
            p[BUFFER_SIZE / 2 + i] = p[i] = permutation[i];
        }
    }
    ...
    public static double noise(double rx, double ry, double rz) {
        ...
        double z = rz;
        final int maxN = 255;
        int x1 = (int) Math.floor(x) & maxN; // FIND UNIT CUBE THAT
        int y1 = (int) Math.floor(y) & maxN;
        int z1 = (int) Math.floor(z) & maxN;// CONTAINS POINT.
        ...
        int a = p[x1] + y1;
        int aa = p[a] + z1;
        int ab = p[a + 1] + z1; // HASH COORDINATES OF
        int b = p[x1 + 1] + y1;
        int ba = p[b] + z1;
        int bb = p[b + 1] + z1; // THE 8 CUBE CORNERS,
