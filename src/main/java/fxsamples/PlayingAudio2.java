 ...
 * @author cdea
 */
public class PlayingAudio2 extends Application {
		...
		mediaPlayer
				.setAudioSpectrumListener((double timestamp, double duration, float[] magnitudes, float[] phases) -> {
					...
					for (int i = 0; i < magnitudes.length; i++) {
						float phase = magnitudes[i];
						double d = phase;
						Rectangle circle = new SimpleRectangleBuilder().width(j).x(10. + i * j)
								.height(y + d).y(y * 3 / 4 - d)
								.fill(Color.rgb(i / j * 255 / phases.length, 255, green, 7. / 10)).build();
	...
	private Node createButtonPanel() {
		...
		SVGPath pauseBackground = new SimpleSvgPathBuilder()
				.content("M 4,3 v11h4v-11z M 10,3 v11h4v-11z M9 17 a 9 9 0 1 1 0.1 0").styleClass("pause-circle")
