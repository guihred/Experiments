...
public class PhotoViewer extends Application {
    ...
    /**
     * Adds the URL string representation of the path to the image file. Based on a
     * URL the method will check if it matches supported image format.
     * 
    ...
    /*
     * Returns a custom created button panel containing left and right buttons to
    ...
    private Group createButtonPanel(Scene scene) {
        ...
        scene.setOnMouseEntered(
            me -> new SimpleSequentialTransitionBuilder().addFadeTransition(DURATION_MILLIS, buttonGroup, 0, 1)
                .build()
                .play());
        // Fade out button controls
        scene.setOnMouseExited(
            me -> new SimpleSequentialTransitionBuilder().addFadeTransition(DURATION_MILLIS, buttonGroup, 1, 0)
                .build()
                .play());
     ...
     * spawns a new thread. If another process is already loading the method will
     ...
     * dropped onto the scene. This will load the image into the current image view
     * area.
     * 
     * @param scene The primary application scene.
     */
