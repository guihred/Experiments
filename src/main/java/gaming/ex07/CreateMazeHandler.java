...
public class CreateMazeHandler implements EventHandler<ActionEvent> {
    ...
    private void setSides() {
        for (int i = 0; i < createdMaze.length; i++) {
            for (int j = 0; j < createdMaze[i].length; j++) {
                ...
                setSides(mazeSquare, i, j);
            }
        }
    }

    private void setSides(MazeSquare mazeSquare, int i, int j) {
        if (i > 0 && andNot(mazeSquare.isEast(), mazeSquare.isNorth(), mazeSquare.isWest())) {
            createdMaze[i][j].setNorth(true);
            createdMaze[i - 1][j].setSouth(true);
        }
        if (i < createdMaze.length - 1 && andNot(mazeSquare.isEast(), mazeSquare.isSouth(), mazeSquare.isWest())) {
            createdMaze[i][j].setSouth(true);
            createdMaze[i + 1][j].setNorth(true);
        }
        if (j < createdMaze[i].length - 1 && andNot(mazeSquare.isNorth(), mazeSquare.isEast(), mazeSquare.isSouth())) {
            createdMaze[i][j].setEast(true);
            createdMaze[i][j + 1].setWest(true);
        }
        if (j > 0 && andNot(mazeSquare.isNorth(), mazeSquare.isWest(), mazeSquare.isSouth())) {
    ...
    private static boolean andNot(Boolean... a) {
        return Stream.of(a).allMatch(b -> !b);
    }
}