...
public class CatanModel {
    ...
    private boolean containsEnough(final Combination combination, final List<CatanCard> list) {
		List<ResourceType> collect = list.stream().map(CatanCard::getResource).filter(Objects::nonNull)
				.collect(Collectors.toList());
		
		List<ResourceType> resources = combination.getResources().stream().collect(Collectors.toList());
		for (int i = 0; i < resources.size(); i++) {
			boolean remove = collect.remove(resources.get(i));
			if(!remove) {
				return false;
			}
		}
		...
		if (containsEnough) {
			...
			switch (combination) {
				...
				case DEVELOPMENT:
					...
					list.add(new CatanCard(values[new Random().nextInt(values.length)]));
					break;
