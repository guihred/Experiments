...
public class CatanModel {
	...
	public CatanModel(final StackPane root, final Pane value) {
		center = root;
		right = value;
		addTerrains(center);
		...
		right.getChildren().add(new HBox(new VBox(userImage, userPoints, new HBox(dice1, dice2)),
				new VBox(availablePorts, vBox), largestArmy, longestRoad));
	...
	private boolean containsPort(final long totalCards, final List<ResourceType> distinct,
			final long differentTypesNumber) {
	...
	private void exchangeForOneResource(final ResourceType selectedType) {
	...
	private Boolean isDealUnfeasible(final Deal deal) {
	...
	private void makeDealButton(final ResourceType selectedType) {
	...
	private void monopolyOfResource(final ResourceType selectedType) {
	...
	private void onSelectResource(final ResourceType selectedType) {
