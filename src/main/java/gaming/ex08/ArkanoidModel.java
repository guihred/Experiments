...
public class ArkanoidModel {
    ...
    public ArkanoidModel(Group group, Scene scene) {
        ...
        int rows = 6;
        group.getChildren().addAll(range(0, cols * rows).mapToObj(i -> newRect(cols, spacing, i)).collect(toList()));
        ...
        Timeline timeline = new SimpleTimelineBuilder()
            .addKeyFrame(new KeyFrame(Duration.millis(10), event -> gameLoop())).cycleCount(Animation.INDEFINITE)
            .build();
        timeline.play();
    }
