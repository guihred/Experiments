...
import static extract.MusicHandler.findImage;
import static extract.MusicHandler.splitAndSave;
import static extract.MusicHandler.splitAudio;
...
public class EditSongController extends Application {
    ...
    public void initialize() {
        ...
        mediaPlayer.get().currentTimeProperty().addListener(e -> updateCurrentSlider(mediaPlayer.get(), currentSlider));
    ...
    public void onActionFindImage(ActionEvent e) {
        ...
        findImage(selectedItem, stage, mediaPlayer);
    }
    ...
    public void onActionSplit(ActionEvent e) {
        ...
        if (initialSlider.getValue() != 0 || finalSlider.getValue() != 1) {
            splitAndSave(selectedItem, initialSlider, finalSlider, outFile, progressIndicator12, stage, mediaPlayer);
            return;
        }
    ...
    public void onActionSplitMultiple() {
        splitAudio(mediaPlayer, selectedItem.getArquivo(), currentSlider, startTime);
    }
    ...
    public void update(ObservableValue<? extends Boolean> o, Boolean oldValue, Boolean newValue) {
        updateMediaPlayer(mediaPlayer.get(), currentSlider, newValue);
    }
