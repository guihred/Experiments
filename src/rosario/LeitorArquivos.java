...
public class LeitorArquivos {
	...
	public static void main(String[] args) throws IOException {
		File file = new File("C:/Users/Guilherme/Downloads/outros/rosario/LOJA.pdf");
		getMedicamentosRosario(file);
	}
		...
		for (int i = 0; i < linhas.length; i++) {
			try {
				...
				if (split.length == 4 || split.length == 3) {
					medicamento.setLote(!StringUtil.isNumeric(split[0]) ? split[0] : Integer.toString(Integer
							.valueOf(split[0])));
					...
					medicamento.setNome(Stream.of(split).skip(1).limit(split.length - 2)
		...
		for (int i = 0; i < linhas.length; i++) {
			try {
				...
				if (split.length > 2) {
					...
					medicamento.setNome(Stream.of(split).skip(1).limit(split.length - 2)
							.collect(Collectors.joining(" ")));
					medicamento.setQuantidade(Integer.valueOf(split[split.length - 1].replace(",00", "").replace(".",
							"")));
					arrayList.add(medicamento);
				}
			} catch (Exception e) {
		...
		while (iterator.hasNext()) {
			...
			try {
				...
				if (cell0.getCellType() == Cell.CELL_TYPE_NUMERIC) {
					registro = Integer.toString((int) cell0
							.getNumericCellValue());
				} else {
	...
	public static void exportarArquivo(ObservableList<Medicamento> medicamentosLoja,
			List<Medicamento> medicamentosSNGPC,
			ObservableList<Medicamento> medicamentosAnvisa) throws IOException {
