...
public class FXEngineCatanTest extends AbstractTestExecution {
    ...
    public void testaToolsVerify() throws Exception {
        show(CatanApp.class);
        List<EdgeCatan> allEdge = lookup(EdgeCatan.class::isInstance).queryAllAs(EdgeCatan.class)
            .stream()
        ...
        List<ButtonBase> allButtons = lookup(".button").queryAllAs(ButtonBase.class)
            .stream()
            .collect(Collectors.toList());
        allButtons.addAll(lookup(".toggle-button").queryAllAs(ToggleButton.class));
        Set<ButtonBase> clickedButtons = new HashSet<>();
        List<Terrain> allTerrains = lookup(Terrain.class::isInstance).queryAllAs(Terrain.class)
            .stream()
            .collect(Collectors.toList());
    ...
    private void clickButton(List<ButtonBase> allButtons, Set<ButtonBase> clickedButtons) {
        ...
        allButtons.addAll(lookup(".button").queryAllAs(ButtonBase.class)
            .stream()
            .filter(t -> !allButtons.contains(t))
            .collect(Collectors.toList()));
        allButtons.stream()
            .filter(e -> !e.isDisabled())
            .filter(e -> e.getParent() != null)
            .filter(ButtonBase::isVisible)
            .filter(e -> e.getParent().isVisible())
            .sorted(Comparator.comparing(clickedButtons::contains))
            .findFirst()
            .ifPresent(t -> {
    ...
    private void clickCards() {
        List<CatanCard> allCards = lookup(CatanCard.class::isInstance).queryAllAs(CatanCard.class)
            .stream()
            .collect(Collectors.toList());
    ...
    private void clickCities(final List<City> cities) {
        List<City> notClickedVillages = lookup(City.class::isInstance).queryAllAs(City.class)
            .stream()
            .filter(t -> !cities.contains(t))
            .collect(Collectors.toList());
        ...
        SettlePoint remove = settlePoints.parallelStream()
            .filter(e -> e.isSuitableForCity(next))
            .findAny()
    ...
    private void clickThiefs(final List<Terrain> queryAllAs) {
        ...
        lookup(Thief.class::isInstance).queryAllAs(Thief.class)
            .stream()
            .filter(e -> e.getParent() instanceof StackPane)
            .forEach(e -> {
                moveTo(e);
                drag(e, MouseButton.PRIMARY);
    ...
    private void clickVillages(final List<Village> allVillages, final List<SettlePoint> settlePoints) {
        List<Village> notClickedVillages = lookup(Village.class::isInstance).queryAllAs(Village.class)
            .stream()
            .filter(v -> !allVillages.contains(v))
            .collect(Collectors.toList());
        ...
        drag(next, MouseButton.PRIMARY);
        SettlePoint remove = settlePoints.parallelStream()
            .filter(e -> e.pointAcceptVillage(next))
            .findAny()
            .orElse(settlePoints.get(0));
        moveTo(remove);
        drop();
    ...
    private Long orderByResources(List<CatanCard> cardsToSelect, final CatanCard c) {
        return -cardsToSelect.stream()
            .filter(e -> e.getResource() == c.getResource())
